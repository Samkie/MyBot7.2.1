...
Func getMyArmyCapacity($bOpenArmyWindow = False, $bCloseArmyWindow = False)
	...
	If $debugsetlogTrain = 1 Or $debugSetlog = 1 Then SETLOG("Begin getMyArmyCapacity:", $COLOR_DEBUG1)
	...
	If $TotalCamp > 0 Then
		SetLog("Troops: " & $CurCamp & "/" & $TotalCamp & " (" & Int($CurCamp / $TotalCamp * 100) & "%)")
		$ArmyCapacity = Int($CurCamp / $TotalCamp * 100)
	Else
		SetLog("Troops: " & $CurCamp & "/" & $TotalCamp)
		$ArmyCapacity = 0
	EndIf

	If $CurCamp >= Int(($TotalCamp * $fulltroop) / 100) Then
		$fullArmy = True
	Else
		$fullArmy = False
	EndIf
