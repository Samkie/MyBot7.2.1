...
Func IsSearchModeActive($iMatchMode, $nocheckHeroes = False, $bNoLog = False)
	...
	Local $currentArmyFull = ($CurCamp >= ($TotalCamp * $fulltroop / 100))
	...
	Local $checkHeroes = Not ($iHeroWait[$iMatchMode] > $HERO_NOHERO And (BitAND($iHeroAttack[$iMatchMode], $iHeroWait[$iMatchMode], $iHeroAvailable + $iHeroUpgradingBit) = $iHeroWait[$iMatchMode]) = False) Or $nocheckHeroes

	If $debugsetlog = 1 Then SetLog($sModeText[$iMatchMode] & " - $checkHeroes: " & $checkHeroes)

	Local $totalSpellsToBrew = 0
	Local $bSpellOk = False
	...
	Else
		...
		($LSpellComp * 2) + ($RSpellComp * 2) + ($HSpellComp * 2) + ($JSpellComp * 2) + ($FSpellComp * 2) + ($CSpellComp * 4)
	EndIf
	;---
	...
	Local $checkSpells = ($bFullArmySpells And $iEnableSpellsWait[$iMatchMode] = 1) Or $iEnableSpellsWait[$iMatchMode] = 0 Or $bSpellOk
...
#CE
	...
	If $checkHeroes And $checkSpells And $checkCCTroops Then ;If $checkHeroes Then
		...
		Else
			...
			Return False
		EndIf
	ElseIf $checkHeroes = False Then
		If $debugsetlog = 1 Then Setlog("Heroes not ready", $COLOR_DEBUG)
		Return False
	ElseIf $checkSpells = False Then
		If $debugsetlog = 1 Then Setlog("Spells not ready", $COLOR_DEBUG)
		Return False
	Else
		If $debugsetlog = 1 Then Setlog("CC troops not ready", $COLOR_DEBUG)
		Return False
	EndIf
