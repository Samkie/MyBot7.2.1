...
Func WaitForClouds()
	...
	While _CheckPixel($aNoCloudsAttack, $bCapturePixel) = False ; loop to wait for clouds to disappear
		...
		If _ColorCheck(_GetPixelColor(430, 333 + $midOffsetY , $bCapturePixel), Hex(0xFF8300, 6), 20) = True Then
		...
		If _ColorCheck(_GetPixelColor(160, 380,$bCapturePixel), Hex(0xFFFFFF, 6),5) And _ColorCheck(_GetPixelColor(699, 380,$bCapturePixel), Hex(0xFFFFFF, 6),5) Then
			AndroidBackButton()
			If _Sleep(1000) Then Return
		EndIf
		; launch attack button and chat button found, back to main?
		If _ColorCheck(_GetPixelColor($aButtonOpenLaunchAttack[4], $aButtonOpenLaunchAttack[5],$bCapturePixel), Hex($aButtonOpenLaunchAttack[6], 6),$aButtonOpenLaunchAttack[7]) And _
		_ColorCheck(_GetPixelColor($aButtonClanWindowOpen[4], $aButtonClanWindowOpen[5],$bCapturePixel), Hex($aButtonClanWindowOpen[6], 6),$aButtonClanWindowOpen[7]) Then
