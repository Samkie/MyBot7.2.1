...
Func CustomSpells()
	...
	While $bLoopFlag
		...
		If $iBrewSpellTime <= 0 Then
			...
			Else
				If _ColorCheck(_GetPixelColor(586, 100 + $g_iMidOffsetY, True), Hex(0X5E5748, 6), 20) = False Then ;color green arrow > not appear
					If $iSamM0dDebug = 1 Then Setlog("Check on brew spells, is that stuck...")
					If gotoBrewSpells() = False Then Return
					If _Sleep(1000) Then Return
		...
		For $i = $enumLightning To $enumSkeleton
			...
			If $iSamM0dDebug = 1 Then SetLog("$tempComp: " & $tempComp)
			If $iSamM0dDebug = 1 Then SetLog("$tempCur: " & $tempCur)
			If $iSamM0dDebug = 1 Then SetLog("$tempOnBrew: " & $tempOnBrew)
			If $iSamM0dDebug = 1 Then SetLog("$tempPre: " & $tempPre)
		...
		If $iSamM0dDebug = 1 Then setlog("$iTotalSpellCapacity: " & $iTotalSpellCapacity)
		If $iSamM0dDebug = 1 Then setlog("$iTotalPreBrewSpellCapacity: " & $iTotalPreBrewSpellCapacity)
		...
		For $i = $enumLightning To $enumSkeleton
			...
			If $tempComp <> $tempCur + $tempOnBrew Then
				...
				If $DifFlag > 0 Then
					If $iSamM0dDebug = 1 Then SetLog("Some spell haven't brew: " & $MySpells[$i][0])
					If $iSamM0dDebug = 1 Then SetLog("Setting Qty Of " & $MySpells[$i][0] & " Spell: " & $tempComp)
				...
				ElseIf $DifFlag < 0 Then
					If $iSamM0dDebug = 1 Then SetLog("Some spell over brew: " & $MySpells[$i][0])
					If $iSamM0dDebug = 1 Then SetLog("Setting Qty Of " & $MySpells[$i][0] & " Spell: " & $tempComp)
					If $iSamM0dDebug = 1 Then SetLog("Current Qty Of " & $MySpells[$i][0] & " Spell: " & $tempComp - $DifFlag)
					If $iSamM0dDebug = 1 Then setlog("OnBrew: " & $tempOnBrew,$COLOR_RED)
					If $iSamM0dDebug = 1 Then setlog("SpellComp: " & $tempComp,$COLOR_RED)
					If $tempOnBrew > 0 Then
		...
		If $iSamM0dDebug = 1 Then SetLog("$bAddFlag: " & $bAddFlag)
		If $iSamM0dDebug = 1 Then SetLog("$bRemFlag: " & $bRemFlag)
		...
		Else
			...
			If IsArray($aPreTrainSpell) Then ;remove all pretrain spell for train onbrew first
				...
				For $i = UBound($aPreTrainSpell) - 1 To 0 Step - 1
					If $iSamM0dDebug = 1 Then SetLog("$aPreTrainSpell: " & $aPreTrainSpell[$i][0] & "," & $aPreTrainSpell[$i][1] & "," & $aPreTrainSpell[$i][2])
					RemoveSpell($aPreTrainSpell[$i][1],$aPreTrainSpell[$i][2])
					If _sleep(250) Then Return
				Next
			EndIf
		...
		If $bAddFlag Then
			...
			If $iSamM0dDebug = 1 Then SetLog("Start brew spell that assign with 'Add' ")
			...
			For $i = 0 To UBound($tempSpells) - 1
				...
				If $tempSpell > 0 Then
					...
					If $iSamM0dDebug = 1 Then SetLog("$iCost: " & $iCost)
						...
						If $iSamM0dDebug = 1 Then SetLog("$BuildCost: " & $iBuildCost)
					If $iSamM0dDebug = 1 Then SetLog("Total need: " & ($tempSpell * $iCost))
...
Func getBrewSpellCapacity()
	If $iSamM0dDebug = 1 Or $g_iDebugSetlog = 1 Then SETLOG("Begin getBrewSpellCapacity:", $COLOR_DEBUG1)
